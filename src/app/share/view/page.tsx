...
function ShareContent() {
  ...
  useEffect(() => {
    async function load() {
      if (id) {
        ...
        if (data) {
          setReport(data)
          // Se tiver código de acesso, ativa a trava
          if (data.access_code && data.access_code.trim() !== "") {
            setIsLocked(true)
          } else {
            // Se não tiver código, já carrega as conexões
  ...
  // TELA DE BLOQUEIO (Só sai daqui se unlocked for true)
        ...
        <Card className="w-full max-w-md p-8 border-none shadow-2xl text-center space-y-6 animate-in zoom-in-95 duration-300">
          ...
          <div className="space-y-4">
            <Input 
              ...
              className={cn("text-center text-2xl font-black h-16", error && "border-red-500 animate-shake")}
              value={pin}
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] selection:bg-indigo/10 selection:text-indigo pb-24 animate-in fade-in duration-1000">
      ...
      <main className="max-w-3xl mx-auto px-6 py-16 md:py-24">
        <article className="space-y-12">
          ...
          {/* O conteúdo só renderiza aqui se unlocked for true */}
